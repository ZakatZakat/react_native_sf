...
/** Ячейка плаката в полосе лендинга. Живёт здесь, потому что её рисуют ДВА
 *  экрана — сам лендинг и превью в /cs/admin/landing («как на лендинге»);
 *  когда значения были скопированы руками, любая правка ломала это обещание.
 *
 *  Почему 1:1, а не 3:4 (как было): замер 34 живых постеров дал медиану
 *  пропорции 1.0 — афиши в массе квадратные, а не вертикальные. Вертикальная
 *  ячейка оставляла в среднем 25.6% пустого поля, квадратная — 15.5%.
 *  Обрезка (cover) не вариант: 26 из 35 постеров потеряли бы больше четверти
 *  площади, а на афише площадь = текст (дата, адрес, состав).
 *
 *  Мат чёрный, а не серый: остаток поля неизбежен при любой ровной сетке, но
 *  чёрный в брутальной вёрстке читается рамкой, а серый — недосмотром. */
export const POSTER_CELL_RATIO = "1 / 1"
export const POSTER_CELL_MAT = CS.K
