...
const KEYFRAMES = `
  ...
  /* The poster sets the card height: full width at its own aspect ratio, so it
     fills edge-to-edge with NO crop and NO grey letterbox bars. max-height caps
     only pathologically tall posters (then object-fit crops the excess). */
  .cs-pola-img { width: 100%; overflow: hidden; border-bottom: 2px solid #0D0D0D; background: #E4E4E1; }
  .cs-pola-img img { width: 100%; height: auto; max-height: 230px; object-fit: cover; display: block; }
  ...
  /* taller box + WHOLE logo/photo (contain) so emblems aren't cropped; the
     letterbox is filled with a blurred copy so it reads intentional, not empty. */
  .cs-deck-place-img { position: relative; height: 102px; border-bottom: 2px solid #0D0D0D; overflow: hidden; background: #E4E4E1; }
  .cs-deck-place-img .cs-deck-place-bg { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover; transform: scale(1.18); filter: blur(13px) brightness(0.94); display: block; }
  .cs-deck-place-img .cs-deck-place-fg { position: absolute; inset: 0; width: 100%; height: 100%; object-fit: contain; display: block; z-index: 1; }
  ...
  .cs-deck-place-blurb { font-family: var(--cs-font-sans); font-size: 8px; line-height: 1.32; color: rgba(13,13,13,0.68); margin-top: 4px; display: -webkit-box; -webkit-line-clamp: 10; -webkit-box-orient: vertical; overflow: hidden; }
  .cs-deck-blurb { font-family: var(--cs-font-sans); font-size: 9px; line-height: 1.38; color: rgba(13,13,13,0.66); margin-top: 6px; display: -webkit-box; -webkit-line-clamp: 5; -webkit-box-orient: vertical; overflow: hidden; }
  ...
  .cs-deck-nav { display: inline-flex; align-items: center; gap: 7px; margin-top: 20px; background: #0D0D0D; padding: 5px 7px; box-shadow: 3px 3px 0 #0055FF; }
          ...
          <div style={{ padding: "13px 14px 0" }}>
            ...
            {ev.tags && ev.tags.length > 0 && (
              <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 10 }}>
                {ev.tags.map((tg) => (
                  <span key={tg} style={{ fontFamily: FONT_SANS, fontSize: 10, fontWeight: 800, letterSpacing: "0.03em", textTransform: "uppercase", color: "#fff", background: CS.B, border: `2px solid ${CS.K}`, boxShadow: `2px 2px 0 ${CS.K}`, padding: "4px 8px", whiteSpace: "nowrap" }}>{tg}</span>
                ))}
              </div>
            )}
